...
export function appendMessageToViewport(message) {
  ...
  const groupShowNames = settings.groupShowCharacterNames ?? true;
  const groupColorCode = settings.groupColorCodeCharacters ?? true;

  // Format timestamp
  const timestamp = formatTimestamp(message.timestamp);

  // For group chats, show character name above their messages (first in sequence only)
  // Respects the groupShowCharacterNames setting
  const showCharacterHeader = inGroupChat &&
    groupShowNames &&
