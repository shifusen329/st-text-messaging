...
/**
 * Gets the group's activation strategy (reply order) from ST settings
 * @returns {string} 'natural', 'list', 'pooled', or 'manual'
 */
function getGroupActivationStrategy() {
  const context = getContext();
  if (!context.groupId) return 'list';

  const groups = context.groups || [];
  const group = groups.find(g => g.id === context.groupId);

  // ST stores activation strategy on the group object
  // Values: 0 = Natural, 1 = List, 2 = Pooled (based on ST source)
  const strategy = group?.activation_strategy ?? 1;
...
/**
 * Gets unmuted group members
 * @returns {Array} Array of unmuted character objects
 */
function getUnmutedGroupMembers() {
  const context = getContext();
  if (!context.groupId) return [];
  ...
  if (!group || !group.disabled_members) return members;

  // Filter out muted (disabled) members
  return members.filter(member => {
    const memberId = member.avatar || member.name;
    return !group.disabled_members.includes(memberId);
  });
}
...
function getCharactersWhoSpokeSinceLastUser() {
  ...
  // Iterate from newest to oldest, stop at last user message
 ...
 * Respects ST's group activation strategy (reply order) settings
 * @returns {Object|null} Character object or null
 */
function getActiveGroupCharacter() {
  const context = getContext();
  const members = getGroupMembers();
  const unmutedMembers = getUnmutedGroupMembers();

  if (members.length === 0) return null;
  if (unmutedMembers.length === 0) {
    console.warn('[phone-ui] All group members are muted');
    return members[0]; // Fallback to first member
  }

  const strategy = getGroupActivationStrategy();
  const messages = getMessages();
  const lastCharMessage = [...messages].reverse().find(m => m.sender === 'character');

  console.log(`[phone-ui] Group activation strategy: ${strategy}`);

  switch (strategy) {
    case 'natural': {
      // Natural order: Check for name mentions in last message, then use talkativeness
      const lastUserMessage = [...messages].reverse().find(m => m.sender === 'user');
      if (lastUserMessage && lastUserMessage.text) {
        const text = lastUserMessage.text.toLowerCase();
        // Check for name mentions (whole words)
        for (const member of unmutedMembers) {
          const nameRegex = new RegExp(`\\b${member.name.toLowerCase()}\\b`);
          if (nameRegex.test(text)) {
            console.log(`[phone-ui] Natural order: ${member.name} mentioned`);
            return member;
          }
        }
      }
      // No mention found - use talkativeness or random selection
      // ST uses talkativeness value (0-100), higher = more likely to speak
      // For now, weighted random based on talkativeness
      const candidatesWithTalk = unmutedMembers.map(m => ({
        member: m,
        talkativeness: m.talkativeness ?? 50 // Default 50%
      }));
      // Filter by talkativeness threshold (random chance)
      const activeByTalk = candidatesWithTalk.filter(c =>
        Math.random() * 100 < c.talkativeness
      );
      if (activeByTalk.length > 0) {
        const selected = activeByTalk[Math.floor(Math.random() * activeByTalk.length)];
        console.log(`[phone-ui] Natural order: ${selected.member.name} by talkativeness`);
        return selected.member;
      }
      // Fallback to random unmuted member
      const randomMember = unmutedMembers[Math.floor(Math.random() * unmutedMembers.length)];
      console.log(`[phone-ui] Natural order: ${randomMember.name} random fallback`);
      return randomMember;
    }

    case 'list': {
      // List order: Follow the member list sequence
      if (lastCharMessage && lastCharMessage.characterId) {
        const lastCharIndex = unmutedMembers.findIndex(m =>
          m.avatar === lastCharMessage.characterId ||
          m.name === lastCharMessage.characterId
        );
        if (lastCharIndex !== -1 && unmutedMembers.length > 1) {
          const nextMember = unmutedMembers[(lastCharIndex + 1) % unmutedMembers.length];
          console.log(`[phone-ui] List order: ${nextMember.name} (next in list)`);
          return nextMember;
        }
      }
      console.log(`[phone-ui] List order: ${unmutedMembers[0].name} (first in list)`);
      return unmutedMembers[0];
    }

    case 'pooled': {
      // Pooled order: Cycle through characters who haven't spoken since last user message
      const spokenSinceUser = getCharactersWhoSpokeSinceLastUser();
      const unspoken = unmutedMembers.filter(m => {
        const memberId = m.avatar || m.name;
        return !spokenSinceUser.has(memberId);
      });

      if (unspoken.length > 0) {
        // Pick random from those who haven't spoken
        const selected = unspoken[Math.floor(Math.random() * unspoken.length)];
        console.log(`[phone-ui] Pooled order: ${selected.name} (hasn't spoken yet)`);
        return selected;
      }
      // All have spoken, pick random from unmuted
      const randomMember = unmutedMembers[Math.floor(Math.random() * unmutedMembers.length)];
      console.log(`[phone-ui] Pooled order: ${randomMember.name} (all spoke, random)`);
      return randomMember;
    }

    default: {
      // Fallback to list order
      console.log(`[phone-ui] Unknown strategy, using list order`);
      return unmutedMembers[0];
    }
  }
}
