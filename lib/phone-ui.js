...
/**
 * Generates character response using SillyTavern's native generation
 * This respects all ST group settings (reply order, generation mode, auto mode, etc.)
 */
async function generateCharacterResponse() {
  ...
  try {
    // Use ST's native generation via /trigger command
    // This inherits all group chat settings automatically
    if (typeof context.executeSlashCommandsWithOptions === 'function') {
      // Use /trigger which respects group reply order, generation mode, etc.
      await context.executeSlashCommandsWithOptions('/trigger');

      // The response will be added to the main chat by ST
      // We need to capture it and add to phone UI
      ...
      if (lastMessage && !lastMessage.is_user) {
        ...
        // Add to phone message store
        const characterMessage = addMessage({
          sender: 'character',
          text: lastMessage.mes.replace(/^📱\s*/, ''), // Strip phone emoji if present
          characterId: character?.avatar || charName,
    ...
    // Fallback: Use generateQuietPrompt if executeSlashCommandsWithOptions not available
    if (typeof context.generateQuietPrompt !== 'function') {
      throw new Error('SillyTavern generation API not available');
    }

    // Get user name
    const userName = context.name1 || 'User';

    // Get responding character info
    let charName, charId, avatarUrl;

    if (inGroup) {
      // For groups, get the first unmuted member as fallback
      const members = getGroupMembers();
      const activeChar = members[0];
      if (!activeChar) {
        console.warn('[phone-ui] No characters in group');
        hideTypingIndicator();
        return;
      }
