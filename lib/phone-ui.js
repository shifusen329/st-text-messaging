...
export function openPhoneUI() {
  ...
  // Update contact name and avatar in header
...
export function appendMessageToViewport(message) {
  ...
  // Build message HTML - simple bubbles without avatars (avatar is in header)
  const messageHtml = `
    <div class="message-row ${message.sender}" data-message-id="${message.id}">
...
function buildPhoneConversationPrompt() {
  ...
  messages.forEach(msg => {
    const sender = msg.sender === 'user' ? userName : charName;
...
async function generateCharacterResponse() {
  ...
  try {
    ...
    // Build the quiet prompt with full context
    const quietPrompt = `You are ${charName} texting with ${userName} on a phone.
...
Now reply as ${charName} via text message. Keep it casual and in-character. Only output ${charName}'s text message reply, nothing else.`;
    ...
    if (response) {
      ...
      // Get character info
      const character = context.characters[context.characterId];
      // Build full avatar URL - ST stores avatars in /characters/ or uses thumbnail endpoint
      const avatarFile = character?.avatar;
      const avatarUrl = avatarFile ? `/thumbnail?type=avatar&file=${encodeURIComponent(avatarFile)}` : '';
      ...
      // Sync to main chat
      await addMessageToMainChat(characterText, false);
